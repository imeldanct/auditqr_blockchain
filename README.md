...
- [Design Decisions](#design-decisions)
  ...
  - [Field Length & Format Constraints](#field-length--format-constraints)
...
A supervisor review asked for a formal database schema table (field, data type, character size) and raised a specific question: why is `smeID` a string rather than an integer? Answering that question properly led to an audit of the whole schema, which surfaced a real gap — no field in the database had any length limit at all.

**What was found:**

Every `String` field in `schema.prisma` had no `@db.VarChar()` attribute, which means Prisma's default PostgreSQL mapping applied: unbounded `TEXT` for every single text column. PostgreSQL was already rejecting the wrong *type* (you cannot insert a number into a text column, or text into a boolean column), but nothing was rejecting the wrong *length* — a 10,000-character string could be submitted to a field like `codeValue`, which should only ever be 4 digits, and the database would accept it without complaint. The only length check anywhere in the codebase was a password minimum of 8 characters, enforced only on the reset and change-password flows — not on registration itself.

**Why `String` (not `Int`) for IDs:**

Every primary key and foreign key in this schema is a UUID, generated by Prisma's `@default(uuid())`. This is deliberate, not incidental: the Parent and Child QR codes generated by this system are printed and scanned by the public, so their identifiers appear directly in URLs anyone can photograph. A sequential integer ID (`1`, `2`, `3`...) would let anyone who scans one QR code guess and enumerate every other product in the system — an Insecure Direct Object Reference (IDOR) risk. A UUID cannot be guessed. This also means every ID across every table is consistently 36 characters (32 hex characters + 4 hyphens), which is what makes the fixed-length constraint below possible.

**Constraints added (`@db.VarChar(n)` in `schema.prisma`):**

| Field(s) | Length | Basis |
|---|---|---|
| Every UUID (all primary keys, all foreign keys, `AuthToken.token`) | 36 | Fixed — a UUID string is always exactly 36 characters |
| `SME.passwordHash` | 60 | Fixed — bcrypt output is always exactly 60 characters |
| `HandoffCode.codeValue` | 4 | Fixed — generated as a random 4-digit number |
| `ParentQRCode.genesisTxHash`, `ScanEvent.txHash` | 88 | Fixed — Solana transaction signature max length |
| `SME.rcNumber` | 10 | Checked against the actual mock CAC data (`RC-123456` format, always 9 characters) plus 1 character of headroom |
| `SME.businessName`, `Product.productName` | 150 | Free text with genuine real-world variability — generous but not unbounded |
| `SME.email` | 255 | RFC standard maximum email length |
| `Product.category`, `SME.businessState` | 50 | Short descriptive words, checked against real category/state examples |
| `SME.businessAddress`, `ScanEvent.gpsLocation` | 255 / 150 | Free text, sized against real CAC address and reverse-geocoded location examples |
| `ParentQRCode.qrData`, `ChildQRCode.qrData` | 70 | Checked against the actual generated format (`auditqr://verify?id=<uuid>`), real max 58 characters |
| `ScanEvent.scannerRole` | 20 | Fixed set of values (`transporter` / `retailer`) |
| `Product.description` | unbounded (`TEXT`) | Genuinely variable-length, no natural cap |

**Migration note:** local network access could not reach the Supabase database from the dev environment used to write this migration, so `prisma migrate dev` could not run directly. The SQL was written by hand (`ALTER COLUMN ... TYPE VARCHAR(n)` for each field) and applied manually via the Supabase SQL editor — the same approach used for the `emailVerified` and `businessAddress` migrations earlier in the project.

**Minimum lengths and format checks — added on top of the size constraints, at both the frontend and backend:**

A separate but related question came up: since there's a maximum, should there also be a minimum? The answer split by field type:

- **Free-text fields** (`businessName`, `productName`, `description`, `category`) — yes, a minimum stops near-empty junk input. But the actual numbers were deliberately kept low: `businessName` and `productName` at 3, not 5 as first proposed, because a 5-character minimum would reject real short names that genuinely exist in this domain — brand names like *Zara* or *Nike*, and product names like *Rice*, *Milk*, *Soap*, *Oil*, or *Tea*, all of which are under 5 characters and completely legitimate for an SME tracking physical goods. `category` was set to 4 for the same reason (*Food* is 4 characters). A minimum length does not add security — someone determined to submit junk can type `"aaaaa"` just as easily as `"aa"` — its only purpose is filtering accidental near-empty submissions, so it should be set as low as that purpose requires, not raised for a false sense of "safety."
- **`password`** — minimum 8 (already existed on reset/change flows; added to registration, which had no minimum at all). Maximum set at 72, which is not an arbitrary number: bcrypt has a hard technical limit of 72 bytes — any characters beyond that are silently truncated and ignored by the hashing algorithm itself, so a longer password provides no additional security and the user deserves to know their extra characters aren't doing anything.
- **Structured fields** (`rcNumber`, `email`, `codeValue`) — a numeric min/max range doesn't meaningfully validate these. `rcNumber` is checked against the fixed pattern `RC-######` rather than a length range, because a 9-character string of random letters would pass a length check while still being a fake RC number. `email` is checked against a standard email pattern rather than a length range, because valid emails vary too widely in length for a range to be useful (`a@b.co` and a long corporate address are both valid). `codeValue` is enforced structurally in the UI itself — four separate single-digit inputs on `code.html`, each restricted to numeric input via `inputmode="numeric"` and a regex strip on every keystroke — rather than validated after the fact.
- **System-generated fields** (every UUID, `passwordHash`, `codeValue` as stored, `genesisTxHash`/`txHash`, `qrData`) — no user ever types these directly, so a bound here isn't input validation in the security sense. It's a defensive backstop against a future bug in the backend's own generation logic, or a direct database edit, rather than protection against a malicious user.

**Frontend feedback:** rather than a toast firing on every keystroke once a field hits its character cap (which would be repetitive and annoying), bounded free-text fields show a live "X/max" counter beneath the input that turns red at the limit — visible the whole time, no popup spam. Minimum-length and format errors are still shown as a toast, but only at submit time, since flagging "too short" while someone is still actively typing would be premature.

**Files affected:** `backend/prisma/schema.prisma`, `backend/prisma/migrations/20260908000000_add_field_length_constraints/migration.sql`, `backend/src/utils/validation.ts` (new — shared `isValidLength`, `isValidEmail`, `isValidRcNumber`, `isValidPassword` helpers), `backend/src/controllers/smeController.ts` (`verifyCACEndpoint`, `registerSME`, `resetPassword`, `updatePassword`, `updateProfile`), `backend/src/controllers/productController.ts` (`createProduct`), `frontend/layout/register.html`, `frontend/layout/create_account.html`, `frontend/layout/create_product.html` + `create_product.js`, `frontend/layout/account_settings.html` + `account_settings.js`.
